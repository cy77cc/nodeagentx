...
OpsAgent 是面向 OpsPilot 控制面的主机侧执行与指标采集 Agent，采用 Go 编写，内嵌 Rust 插件运行时，包含两大核心子系统：

- **Telegraf 风格指标采集管线** — Input → Processor → Aggregator → Output 插件架构
- **nsjail 沙箱执行引擎** — 命名空间隔离 + cgroup v2 资源限制 + 安全策略

附加子系统：Gateway（TCP 隧道 + SSH 代理的跳板机）、Checker（5 类系统健康检查）、Plugin Gateway（自定义插件生命周期管理）、Config 热重载、审计日志。
...
> 完整安全加固文档见 [安全加固手册](docs/zh/security-hardening.md)。
...
详见 [平台集成指南](docs/zh/platform-integration-guide.md)，包含：
...
│   │   └── proxy/                #   SSH 代理与命令中继
│   ├── checker/                  # 系统健康检查子系统
│   │   ├── kernel/               #   内核版本、启动参数、模块、sysctl
│   │   ├── filesystem/           #   文件存在、权限、目录权限、挂载选项
│   │   ├── network/              #   网络参数、端口、iptables、SSH 配置
│   │   ├── service/              #   服务检查、用户检查、PAM、cron
│   │   └── container/            #   Docker、containerd、cgroup、容器运行时
...
│   │   ├── gateway-tunnel-guide.md   # 网关隧道指南
│   │   └── changelog.md              # 变更记录
│   ├── en/                       # 英文文档
...
| 文档 | 说明 |
|------|------|
| [快速入门](docs/zh/quickstart.md) | 安装、配置、首次运行指南 |
| [配置参考](docs/zh/config-reference.md) | 完整配置项说明与示例 |
| [API 参考](docs/zh/api-reference.md) | HTTP API 端点与请求/响应格式 |
| [架构设计](docs/zh/architecture.md) | 子系统架构、数据流、设计决策 |
| [平台集成指南](docs/zh/platform-integration-guide.md) | 平台侧开发者指南：gRPC 服务实现、消息交互流程 |
| [插件协议规范](docs/zh/plugin-contract.md) | UDS JSON-RPC 2.0 协议定义 |
| [SDK 开发指南](docs/zh/sdk-development-guide.md) | Go/Rust 插件 SDK 使用指南 |
| [安全加固手册](docs/zh/security-hardening.md) | 安全架构、加固措施、审计配置 |
