...
- 为隧道连接添加 PSK 认证机制
- 强制 SSH 主机密钥验证，禁用 StrictHostKeyChecking=no
- 使用 crypto/rand 生成不可预测的隧道 ID
- 对 SSH 命令参数进行严格转义，防止命令注入

### 安全检查清单

- 将 Gateway 安全要求纳入部署前安全检查清单

---

## 2026-05-23 -- Gateway/Tunnel 子系统

新增 Gateway 与隧道子系统，支持反向隧道和代理模式连接。

### Gateway 核心

- 新增 Gateway 配置：`gateway.enabled`、`listen_addr`、`hosts`
- 实现 PSK 预共享密钥认证
- 支持 Proxy 代理模式，通过 SSH 隧道转发流量
- SSH 密钥管理与 `known_hosts` 主机验证

### 隧道功能

- 基于 WebSocket 的隧道建立与维护
- 隧道心跳与自动重连机制
- 多主机并发隧道支持
...
- 新增 Gateway 隧道指南（中英文）
- 在运维部署指南中添加 Gateway 部署章节
- 在安全加固手册中添加 Gateway 安全章节

---

## 2026-05-07 -- 系统健康检查器

增强健康检查子系统，提供更细粒度的子系统状态监控。

### 健康检查

- 为每个子系统添加独立的 HealthStatus
- 在 /healthz 端点返回各子系统详细状态（healthy / degraded / unhealthy）
- 添加 last_collection 时间戳字段
- 从 Agent 传播版本信息和启动时间到健康端点
- 未认证请求隐藏版本和运行时信息

### 监控

- 新增 /readyz 就绪探针端点
- 健康状态与 Prometheus 指标集成
